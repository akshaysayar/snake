...
def game(button,inputs):
 ...
 #calculate angles
        if button.value==1:
            ...
            if [xs[0],ys[0]-10] in l or ys[0]-10 in y_fence:
                right = 1
            else:
                right =0
            if [xs[0],ys[0]+10] in l or ys[0]+10 in y_fence:
                ...
                left = 0
            if [xs[0]-10,ys[0]] in l or xs[0]-10 in x_fence:
                front = 1
            else:
                front = 0
        ...
        if button.value==3:
            ...
            if [xs[0],ys[0]-10] in l or ys[0]-10 in y_fence:
                ...
                left = 0
            if [xs[0]+10,ys[0]] in l or xs[0]+20 in x_fence:
                front = 1
            else:
                front = 0
...
def printing(button,inputs):
    ...
    model = tflearn.DNN(network, tensorboard_dir='log')
    model.load("/home/akshay/data/personal/Python_projects/Snake/model/version_2_100_games.tflearn")
    training_data = []
    previous =-1
    previous_dir = 3
    ...
    while True:
        if button.value!=17 and len(inputs)!=0 and previous!=inputs[-1][0]:
            ...
            break
    pd.DataFrame(training_data).to_csv("/home/akshay/data/personal/Python_projects/Snake/training_data_3.csv",index=False)
...
def main():
    ...
    p3 = multiprocessing.Process(target=printing, args=(flag_1,inputs))
    p1.start()
