...
def printing(button,inputs):
    ...
    LReg = LReg= pickle.load(open("/home/akshay/data/personal/Python_projects/Snake/model/RF_v2.pkl", 'rb'))
    previous =-1
    previous2= 3
    previous_dir = 3
    action = 3
    while True:
        if button.value!=17 and len(inputs)!=0 and previous!=inputs[-1][0]:
            ...
            action = d_map[str(previous_dir)+str(symbol[0])]
            print("action",action)
            button.value = action
